...
# print(int(np.mean(token_lens))+1)

# MAX_LEN = int(np.mean(token_lens))+1+5+2
MAX_LEN = max(token_lens)+2
    ...
    for j in range(len(lr)):
        for k in range(len(n_layers)):
            for l in range(len(bert_size)):
                ...
                #     project="amass_babel_frame_annotation_plot",

                #     name="best_parameters_bert_unfreezed",
...
model.load_state_dict(torch.load("/home/michieletto/Progetti_per_tesi/Progetto_RNN/Parametri_modello/amass_babel_frame_annotations.pt"))
...
if not zero_velocity:
    EvaluationProtocol.save_all_metrics(
        ...
        metrics_dict=metrics_dict,
        framerate=25,
        csv_path="/home/michieletto/Progetti_per_tesi/Progetto_RNN/Risultati_test_csv/amass_babel_frame_annotations",
        print_table=False,
    )
else:
    EvaluationProtocol.save_all_metrics(
        ...
        metrics_dict=metrics_dict,
        framerate=25,
        csv_path="/home/michieletto/Progetti_per_tesi/Progetto_RNN/Risultati_test_csv/amass_babel_frame_annotations_zero_velocity",
        print_table=False,
    )
